...
export function ShareButton({
  ...
  if (compact) {
    return (
      <button
        ...
        className={`flex h-9 w-9 shrink-0 items-center justify-center border border-line text-secondary transition-colors hover:border-line hover:text-primary ${className}`}
      >
        {copied ? (
  ...
  return (
    <button
      type="button"
      onClick={share}
      className={`shrink-0 border border-line px-3 py-1.5 text-[12.5px] font-semibold text-secondary transition-colors hover:border-line hover:text-primary ${className}`}
    >
      {copied ? "Copied" : "Share"}
    </button>
  );
