...
const TOC: { id: string; label: string }[] = [
  { id: "tldr", label: "TL;DR" },
  { id: "quickpass", label: "Quick pass guide" },
    ...
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      <div className="lg:grid lg:grid-cols-[200px_1fr] lg:gap-10">
        ...
        <main className="min-w-0 space-y-8">
          <Section id="tldr" title="TL;DR">
            <UL>
              <li>
                ...
                threaded chat, live stats, polls, call-ins, recordings - hangs off that live room. The
                platform never shows the match itself; that&apos;s the legal foundation.
              </li>
              <li>
                <b className="text-primary">How it&apos;s built, in one breath:</b> a single Next.js
                app on Vercel. Postgres (Supabase) is the source of truth; Ably fans realtime events
                out to clients; LiveKit carries the audio. Every write is an API route that validates
                (zod) → saves → broadcasts; clients can rebuild full room state from the DB after any
                disconnect. No servers to run, ~40 tables, one daily cron.
              </li>
              <li>
                <b className="text-primary">Where it stands:</b> feature-complete and deployed to
                production, but <em>pre-launch with zero real users</em>. Not built: payments (Stripe
                tipping). Built but switched off pending keys: email + push notifications. Pending:
          ...
          <Section id="quickpass" title="Doing a quick pass? Start here">
            <p>
              This page is a full reference; you don&apos;t need all of it. Pick your depth:
            </p>
            <UL>
              <li>
            ...
            <H3>Where a second pair of eyes is most valuable</H3>
            <p>If you&apos;re here to sanity-check the build rather than join it, these are the honest risk concentrations:</p>
            <ol className="ml-5 list-decimal space-y-1.5">
              <li>
                <b className="text-primary">The audio sync path</b> (<C>components/room/audio/</C>):
                WebRTC → AudioWorklet ring buffer → offset playback, with iOS Safari as the target
                platform. The riskiest tech in the product and the least conventionally testable.
              </li>
              <li>
                <b className="text-primary"><C>RealtimeRoom.tsx</C></b>: one large client orchestrator
                owns every subscription and all room state. Deliberate, but worth an opinion on
                long-term maintainability.
              </li>
              <li>
                <b className="text-primary">The security model</b>: RLS on every table + service-role
                writes only inside API routes after explicit auth checks. If that pattern has a hole
                anywhere, it matters everywhere.
              </li>
              <li>
                <b className="text-primary">Serverless media processing</b>: ffmpeg (recording recut)
                and sharp (avatars) inside Vercel functions - duration/memory ceilings under real load.
              </li>
              <li>
                <b className="text-primary">Rate limiting is per-lambda</b>, not global (known
                limitation; Upstash/Redis is the upgrade path).
              </li>
              <li>
                <b className="text-primary">Vendor coupling</b>: Sportmonks quota (rate-limited plan),
                Ably + LiveKit costs at concurrency - nothing measured under load yet.
              </li>
            </ol>
          </Section>
