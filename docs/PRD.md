...
- 19.5 **Fixture sync expansion.** Sync switches from Arsenal-only to all EPL fixtures (league id 8), horizon today + 120 days. Scheduled trigger: cron every 6 hours (the Phase 7 deferral comes due); the manual admin trigger stays. Sync detects kickoff and status changes (postponed, canceled) and updates fixture rows. *(Revised 2026-07-03 to fit Vercel Hobby's once-daily crons: a daily sync cron, plus an opportunistic refresh when the fixture picker opens against a cache older than 6 hours, plus the manual admin trigger.)*
...
- 21.4 **Delivery.** Outbox table written by the triggering API route; scheduled timings are computed into the outbox at RSVP/schedule time and recomputed when a room moves. *(Delivery revised 2026-07-03 to fit Vercel Hobby's once-daily crons, replacing the every-minute drainer: immediate types send inline from the triggering route after the response (send-after-write); due reminder rows drain opportunistically from high-traffic server routes (home, room, matches) behind a per-instance throttle; a daily cron sweeps retries and anything traffic missed. Known limit, accepted: with zero site traffic a reminder can deliver late. Also: email sends route through one `lib/notify/email.ts` sender that no-ops with a console note until RESEND_API_KEY + a verified domain land; everything else, including web push, is live immediately.)*
...
- 25.6 **Tips.** In a co-hosted room the tip sheet shows both hosts and the listener picks the recipient; each tip routes 100% (minus platform fee) to the chosen host. No splitting. Single-host rooms keep the FR-15 flow untouched. *(2026-07-03: Stripe arrives after the domain/email decision, so FR-15 is not yet built; 25.6 ships as prepped-for-drop-in: tip rows carry recipient_user_id from day one and the recipient picker lands with the tip sheet when Stripe does.)*
