...
8. **End Broadcast** — room → `wrapped`. Recording processing kicks off asynchronously; the **Downloads panel** shows status → full MP3 + per-segment MP3s + a zip, with ±2 min marker adjust + recut. Radio segments are kept for **48h** (they are the recording's source and the recut source); the processed files stay for **7 days**. Download the backup zip promptly after each match; that local copy is the durable archive. A 3h+ show ships as **part files only** (the single full file would exceed the storage size cap) and can take **several automatic passes** to process: the Downloads panel retries a stalled run by itself while it is open (up to 8 attempts; the daily cron catches rooms nobody is watching), so leave it alone unless it says **failed**, and then press Retry once. The Downloads panel also carries **Episode notes** (copy or download .txt) (paste them when uploading the shows in Spotify for Creators, which hosts the podcast) and a **Download backup** button that zips the pre-game, Full match, and post-game files for local archiving.
